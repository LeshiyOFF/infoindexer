 ...
 *
 * @remarks
 * Domain service for calculating adaptive ClickHouse configuration
 ...
 *
 * Follows SRP: responsible only for configuration calculations.
 */
import { MemorySize } from '../value-objects/memory-size.vo';
...
export interface CalculatedConfig {
  readonly maxMemoryUsage: string; // bytes as string
...
export class ResourceCalculationService {
   ...
   */
  calculateBatchSize(totalMemoryGB: number, targetBatches: number = 32): number {
  ...
  validateMinimumRequirements(resources: ResourceInfo): boolean {
    ...
    if (totalGB < this.MIN_MEMORY_GB) {
      throw new Error(
        `Insufficient memory: ${totalGB.toFixed(1)}GB available, ` +
