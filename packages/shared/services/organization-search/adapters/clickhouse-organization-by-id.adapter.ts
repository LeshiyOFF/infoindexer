 ...
 */
export class ClickHouseOrganizationById implements IOrganizationById {
  constructor(
  ...
  private async fetchSummary(id: string): Promise<FinancialSummary | undefined> {
  ...
  private mapRowToFinancialSummary(row: Record<string, unknown>): Result<FinancialSummary, Error> {
  ...
  private async fetchFinancialReports(id: string): Promise<FinancialReport[]> {
    const result = await this.client.query({
    ...
    return ArrayUtil.ensureArray(json) as FinancialReport[];
  }

  private async fetchMetadata(id: string): Promise<CompanyMeta | null> {
    const result = await this.client.query({
    ...
    const json = await result.json();
    const records = ArrayUtil.ensureArray(json) as CompanyMeta[];
    return records.length > 0 ? records[0] : null;
  }

  private async fetchSanctions(id: string): Promise<readonly SanctionDTO[]> {
    try {
      const result = await this.client.query({
      ...
      const json = await result.json();
      const rows = ArrayUtil.ensureArray(json) as Array<{
        readonly id: unknown;
  ...
  private async fetchConnections(meta: CompanyMeta, inn: string): Promise<Partial<CompanyMeta>[]> {
