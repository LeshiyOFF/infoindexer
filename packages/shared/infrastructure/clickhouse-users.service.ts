...
export class ClickHouseUsersService {
  constructor(
    private readonly rbac: IClickHouseRBACManager,
   ...
   */
  async initialize(config: UserInitConfig): Promise<UserInitResult> {
    this.logger.info('Initializing ClickHouse users');

    const details: UserInitResult['details'] = [];
    let createdCount = 0;
    let skippedCount = 0;
   ...
   * Create admin user
   */
  private async createAdminUser(config: UserInitConfig): Promise<UserInitResult['details'][0]> {
   ...
   * Create worker user
   */
  private async createWorkerUser(config: UserInitConfig): Promise<UserInitResult['details'][0]> {
    const result = await this.rbac.createUser({
   ...
   * Create API user
   */
  private async createApiUser(config: UserInitConfig): Promise<UserInitResult['details'][0]> {
    const result = await this.rbac.createUser({
   ...
   */
  private async grantAccess(username: string, database: string): Promise<void> {
 ...
 */
export function createClickHouseUsersService(
  rbac: IClickHouseRBACManager,
  logger: ILogger
