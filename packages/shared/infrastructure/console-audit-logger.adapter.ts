 ...
 * ```
 */
export class ConsoleAuditLoggerAdapter implements IAuditLogger {
   ...
   */
  async logEvent(event: AuditEvent, _options?: AuditLoggerOptions): Promise<AuditLoggerResult> {
   ...
   */
  private addColor(json: string): string {
    const colors = {
      reset: '\x1b[0m',
      dim: '\x1b[2m',
      cyan: '\x1b[36m',
    ...
    return (
      ...
      json.replace(
        /"level": "(\w+)"/,
        `${colors.reset}"level": ${colors.cyan}"$1"${colors.dim}`
      ).replace(
        /"service": "(\w+)"/,
        `${colors.reset}"service": ${colors.green}"$1"${colors.dim}`
      ) +
      colors.reset
    );
  }
