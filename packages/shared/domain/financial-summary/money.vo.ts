...
export class Money {
  ...
  static create(data: MoneyData): Result<Money, InvalidMoneyError> {
    ...
    if (typeof amount !== 'number' || isNaN(amount)) {
      return Result.error(
        new InvalidMoneyError(amount ?? 0, currency ?? 'RUB', 'missing_field')
      );
    }

    if (amount < 0) {
      return Result.error(
        new InvalidMoneyError(amount, currency, 'negative_amount')
      );
    }
    ...
    if (!ALLOWED_CURRENCIES.has(normalizedCurrency)) {
      return Result.error(
        new InvalidMoneyError(amount, currency, 'invalid_currency')
      );
    }
  ...
  add(other: Money): Result<Money, InvalidMoneyError> {
    if (this.currency !== other.currency) {
      return Result.error(
        new InvalidMoneyError(this.amount, `${this.currency}/${other.currency}`, 'invalid_currency')
      );
    }
