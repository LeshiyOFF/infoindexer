...
export class AuditEventValidator {
  ...
  validateResourceId(resourceId: string | null): void {
    if (resourceId !== null) {
      ...
      if (resourceId.length > LIMITS.MAX_RESOURCE_ID_LENGTH) {
        throw new Error(
          `resourceId exceeds max length of ${LIMITS.MAX_RESOURCE_ID_LENGTH}`
        );
      }
    }
  }
  ...
  validateMetadata(metadata: AuditMetadata): void {
    ...
    if (metadataStr.length > LIMITS.MAX_METADATA_SIZE) {
      throw new Error(
        `metadata exceeds max size of ${LIMITS.MAX_METADATA_SIZE} bytes`
      );
    }
  ...
  private validateMetadataValue(key: string, value: unknown): void {
    const allowedTypes = ['string', 'number', 'boolean'];
    if (!allowedTypes.includes(typeof value)) {
      throw new Error(
        `metadata value for key "${key}" must be string, number, or boolean`
      );
    }
  }
}
