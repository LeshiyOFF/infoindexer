...
export class ClickHouseSyncStateAdapter implements ISyncStateStoragePort {
  /**
   ...
   */
  async saveSyncTimestamp(syncType: string, timestamp: Date): Promise<void> {
   ...
   */
  async getRecordsProcessed(syncType: string): Promise<number> {
    const result = await this.client.query({
      query: `
    ...
    const rows = await result.json() as { records_processed: number }[];
