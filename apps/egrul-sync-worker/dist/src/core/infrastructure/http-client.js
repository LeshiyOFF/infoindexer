...
class FTMHttpClient {
    ...
    userAgent;
    resumeCoordinator;
    constructor(proxyAgent, userAgent = 'Mozilla/5.0 (compatible; InfoIndexer/1.0; +https://github.com/)', resumeStorage) {
        this.proxyAgent = proxyAgent;
        this.userAgent = userAgent;
        // Создаём ResumeCoordinator только если передан storage
        this.resumeCoordinator = resumeStorage
            ? new domain_1.ResumeCoordinator(resumeStorage, proxyAgent, userAgent)
            : null;
    }
    /**
    ...
    async fetchStream(url) {
        ...
        const maxAttempts = useProxy ? constants_1.PROXY_RETRIES : 1;
        for (let i = 0; i < maxAttempts; i++) {
            try {
                if (i > 0) {
                    await this.sleep(constants_1.RETRY_DELAY_MS * i);
                ...
                if (!this.isRetryableError(e) || i === maxAttempts - 1) {
                    const hint = e?.message?.includes('127.0.0.1')
     ...
     */
    async fetchStreamWithResume(url) {
    ...
    async fetchDownloadUrl() {
        ...
        const useProxy = !!this.proxyAgent;
        for (let i = 0; i < (useProxy ? constants_1.PROXY_RETRIES : 1); i++) {
            try {
                if (i > 0) {
                    await this.sleep(constants_1.RETRY_DELAY_MS * i);
                    console.log(`Retry ${i}/${constants_1.PROXY_RETRIES} via proxy...`);
                }
                ...
                if (!this.isRetryableError(e) || i === (useProxy ? constants_1.PROXY_RETRIES : 1) - 1) {
                    throw e;
                }
            }
        }
...
exports.FTMHttpClient = FTMHttpClient;
