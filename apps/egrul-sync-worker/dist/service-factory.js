...
const egrul_transform_service_1 = require("./core/services/egrul-transform.service");
const clickhouse_staging_adapter_1 = require("./core/infrastructure/adapters/clickhouse-staging.adapter");
const clickhouse_production_adapter_1 = require("./core/infrastructure/adapters/clickhouse-production.adapter");
const memory_monitor_adapter_service_1 = require("./core/infrastructure/adapters/memory-monitor-adapter.service");
const clickhouse_identity_resolver_adapter_1 = require("./core/infrastructure/adapters/clickhouse-identity-resolver.adapter");
const staging_config_vo_1 = require("./core/domain/value-objects/staging-config.vo");
...
const transform_polling_worker_1 = require("./core/workers/transform-polling.worker");
const worker_config_vo_1 = require("./core/domain/value-objects/worker-config.vo");
const console_logger_adapter_1 = require("./core/infrastructure/adapters/console-logger.adapter");
/**
...
async function initializeServices(clickhouseClient, proxyAgent) {
    ...
    const oldTransformService = new staging_transform_service_1.StagingTransformService(identityResolver);
    const productionStorage = new clickhouse_production_adapter_1.ClickHouseProductionAdapter(clickhouseClient);
    const memoryMonitor = new memory_monitor_adapter_service_1.MemoryMonitorAdapter(clickhouseClient);
    const config = staging_config_vo_1.StagingConfig.forProduction();
    const transformService = new egrul_transform_service_1.EgrulTransformService(clickhouseClient, stagingStorage, productionStorage, memoryMonitor, config);
    const stagingSync = new staging_sync_service_1.StagingSyncService(stagingStorage, transformService);
    // Transform Polling Worker (Iteration 4)
    const transformPollingWorker = new transform_polling_worker_1.TransformPollingWorker(transformService, worker_config_vo_1.WorkerConfig.forProduction(), enableMetrics ? metrics : undefined, new console_logger_adapter_1.ConsoleLoggerAdapter());
