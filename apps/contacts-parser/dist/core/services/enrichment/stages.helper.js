...
exports.EnrichmentStagesHelper = void 0;
 ...
 */
class EnrichmentStagesHelper {
    ddg;
    email;
    phone;
    constructor(ddg, email, phone) {
        this.ddg = ddg;
        this.email = email;
        this.phone = phone;
    }
     ...
     */
    async runDirectorStage(inn, page, contacts, stage, addSource) {
        try {
            ...
            this.email.filterBlocked(emails).forEach(e => contacts.emails.push({ val: e, source: stage.label, type: 'direct' }));
            phones.filter(p => this.phone.isValidLength(p)).forEach(p => contacts.phones.push({ val: p, source: stage.label, type: 'direct' }));
     ...
     */
    async runRegistryStage(inn, page, contacts, stage, addSource) {
        try {
            console.log(`[Waterfall] ${stage.label}: ${inn}...`);
            await shared_1.redisClient.hset(`contacts:status:${inn}`, { status: 'running', stage: stage.stage });
            const { emails, phones } = await this.ddg.searchWithTargetVisit(page, stage.query, 5);
            this.email.filterBlocked(emails).forEach(e => contacts.emails.push({ val: e, source: stage.label, type: 'official' }));
...
exports.EnrichmentStagesHelper = EnrichmentStagesHelper;
