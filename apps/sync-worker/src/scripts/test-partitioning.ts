 ...
 */
async function checkTablePartition(tableName: string): Promise<void> {
  ...
  const parts = await partsResult.json() as Array<{
    partition: string;
    name: string;
    rows: number;
  ...
  const partitionMap = new Map<string, { rows: number; bytes: number }>();
  ...
  const createRows = await createResult.json() as Array<{ create_table_query: string }>;
 ...
 */
async function testDropPartition(): Promise<void> {
  ...
  try {
    ...
    const beforeRows = await beforeResult.json() as Array<{ cnt: string }>;
    ...
    const afterRows = await afterResult.json() as Array<{ cnt: string }>;
