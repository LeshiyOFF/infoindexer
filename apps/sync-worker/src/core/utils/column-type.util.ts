...
export class ColumnTypeUtil {
   ...
   */
  private mapDuckDbTypeToClickHouse(duckdbType: string): string {
    if (duckdbType.includes('VARCHAR')) return 'String';
    if (duckdbType.includes('BIGINT')) return 'Int64';
    if (duckdbType.includes('INTEGER')) return 'Int32';
    if (duckdbType.includes('DOUBLE')) return 'Float64';
    if (duckdbType.includes('FLOAT')) return 'Float32';
    if (duckdbType.includes('BOOLEAN')) return 'UInt8';
    if (duckdbType.includes('DATE')) return 'Date';
    if (duckdbType.includes('TIMESTAMP')) return 'DateTime';
