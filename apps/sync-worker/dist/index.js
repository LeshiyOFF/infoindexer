...
const sync_factory_1 = require("./core/factories/sync.factory");
const active_syncs_manager_1 = require("./active-syncs-manager");
const BATCH_SIZE = parseInt(process.env.BATCH_SIZE || '20000');
const REPORT_INTERVAL = 5;
const factory = new sync_factory_1.SyncFactory();
const activeSyncs = new active_syncs_manager_1.ActiveSyncsManager();
let isShuttingDown = false;
/**
 * Обработчик сообщения sync:start
 */
async function handleSyncStart(message) {
 ...
 */
function handleSyncAbort(message) {
    const payload = JSON.parse(message);
